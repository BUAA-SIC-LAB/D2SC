...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, (q.permute(0, 2, 1, 3).reshape(B, N, C), k.permute(0, 2, 1, 3).reshape(B, N, C),
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, teacher=False,
                 suplayer=4, encoder_pred_channel=768, decoder_pred_channel=512, supervise_all=False):
        super().__init__()
    ...
    def forward(self, imgs, noise, mask_ratio=0.75):
        ...
        # If it is not the teacher, output the classification results.
        if not self.teacher:
            ...
            if not self.supervise_all:
                return pred_decoder_feature[:, 1:, :][
                    mask.bool()], None, mask, prenorm_feature, pred_feature, attn_list_encoder, None
            else:
...
# Variant   | Layers (L) | Hidden Size (D) | Attention Heads (H) | MLP Dim (FFN)
# --------- | ---------- | --------------- | ------------------- | -------------
# ViT-Tiny  | 12         | 192             | 3                   | 768
# ViT-Small | 12         | 384             | 6                   | 1536
# ViT-Base  | 12         | 768             | 12                  | 3072
...
mae_vit_huge_patch14 = mae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 block
